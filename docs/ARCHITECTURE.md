...
Tasuki（襷）は、Claude Code（AI）と人間の開発者が協働するためのレビュー補助ツールである。駅伝の「襷リレー」のように、AI と人間が作業を交互に受け渡しながらゴールに向かうワークフローを実現する。
...
Tasuki はこれらを統合的に解決するデスクトップアプリケーションである。
...
`.git/`、`node_modules/`、`target/` などのディレクトリは無視対象。
...
pnpm tauri dev        # 開発サーバー起動（Vite HMR + Tauri WebView）
```

開発サーバーはポート 1420 で起動し、Tauri WebView がそのサーバーに接続する。
...
ビルドプロセス：
1. `pnpm build` — TypeScript コンパイル + Vite バンドル
2. `cargo build --release` — Rust バックエンドのリリースビルド（LTO 有効、シンボルストリップ）
3. プラットフォーム固有のパッケージング（Windows/macOS/Linux）

最終成果物はスタンドアロンの実行ファイルとなり、Web サーバー不要で動作する。
...
- Rust 1.77+
- pnpm
- OS ごとの Tauri ビルド依存（[Tauri prerequisites](https://v2.tauri.app/start/prerequisites/)）
