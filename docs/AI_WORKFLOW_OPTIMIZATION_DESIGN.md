...
- **Claude Code**: 探索・構造化・実行を担う。既存コード・設計書・技術動向を踏まえて選択肢を提示し、承認された方針に従って実装する
- **Tasuki**: 成果物を検証する。設計書とdiffを並べ、構造化されたフィードバックを出す
- **ファイルシステム**: 状態を保持する。設計書、ハンドオフ、レビュー結果はすべてファイル
- **人間**: 問題を定義し、判断する。「何がしたいか（Why / What）」を伝え、AIが構造化した選択肢に対して最終判断を下す。選択肢の探索・比較・構造化はAIの仕事
...
- 入力: 人間の意図（「何がしたいか」だけでよい）
- AIの責務: 既存コード・設計書・技術的制約を自律的に探索し、選択肢をトレードオフ付きで構造化して提案する
- 出力: `~/.claude/designs/{repo}/{feature}/design.md`（推奨案付き）
- 出力: `~/.claude/designs/{repo}/{feature}/decisions.md`（判断ポイントごとの選択肢・比較・推奨）
...
AIが判断ポイントごとに選択肢を構造化して提示する。人間は結論を承認・変更するだけ。
フォーマット:
  ## {判断事項}
  - 選択肢:
    - A: {選択肢A} — メリット: / デメリット:
...
人間は「何がしたいか」だけを伝える。AIが既存のコードベース・設計書・技術動向を自律的に探索し、最適な方式をトレードオフ付きで提案する。
...
2. 自律的にコンテキストを収集:
   - 既存のコードベース構造を把握
   - 既存の設計書・アーキテクチャドキュメントを読む
   - 関連する技術的制約を特定
3. design.md を作成:
   - 目的とスコープ
   - 技術的アプローチ（複数の選択肢をトレードオフ付きで比較し、推奨案を提示）
   - API / データモデル / 画面設計
   - エッジケース
   - テスト戦略
4. decisions.md を作成:
   - 主要な判断ポイントごとに、検討した選択肢・トレードオフ・推奨案を構造化
   - 人間が「これでいく」と判断できる材料を揃える
5. 完了メッセージ:
   「設計書を作成しました。Tasukiで `tasuki docs` を実行してレビューしてください。
    decisions.mdに判断が必要なポイントをまとめています」
```
...
│ Claude Code:                                                │
│   /design auth                                              │
│   → 既存コード・設計書・技術的制約を自律的に探索              │
│   → 選択肢をトレードオフ付きで比較                           │
│   → design.md（推奨案付き）+ decisions.md を生成             │
│   → 「Tasukiでレビューしてください」と報告                     │
│                                                             │
│ 人間:                                                       │
│   tasuki docs                                               │
│   → design.mdを読む（AIが構造化した提案を確認）              │
│   → decisions.mdで判断ポイントを見て、選択・承認する          │
│   → 気になる点があればコメント                                │
...
### 6.1 根本原則: AIのコンテキスト拡大速度 >> 人間のコンテキスト拡大速度

AIは既存コード・設計書・技術文献を数秒で読み込み、構造化できる。人間が同じことをすると数時間かかる。この非対称性から導かれる役割分担:

- **人間の仕事**: 問題を定義する（Why / What）+ 最終判断を下す
- **AIの仕事**: 探索・比較・構造化する（How の選択肢を提示）

人間が「JWTとセッションベースを比較して」と指示するのは、人間側にその知識があることを前提としており、方向が逆。人間は「認証機能を追加したい」とだけ言えばよく、何と何を比較すべきかはAIが自律的に判断する。

### 6.2 設計フェーズでの変化

**Before**: 「認証機能を作って」→ AIが実装 → レビュー
**After**: 「認証機能を追加したい」→ AIが自律的に探索・比較・提案 → 人間が判断 → 設計確定 → 実装へ

具体的な行動:
- **意図だけを伝える**: 「○○したい」「○○が問題」— Howは指定しない。制約がある場合（既存技術との互換性等）だけ伝える
- **AIの提案を判断する**: decisions.mdに構造化された選択肢を見て「これでいく」と承認する。判断理由を追記する
