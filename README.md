...
│   ├── src/
│   │   ├── git.rs      # git2-rs: diff取得、ログ、ファイル読み取り
│   │   ├── watcher.rs  # notify: ファイル監視
│   │   ├── commands.rs # Tauri コマンド (frontend ↔ backend bridge)
│   │   ├── error.rs    # エラー型定義
│   │   ├── lib.rs      # CLI引数パース、アプリ起動
│   │   └── main.rs     # エントリーポイント
│   └── Cargo.toml
├── src/                # React + TypeScript frontend
│   ├── components/
│   │   ├── Toolbar.tsx       # 表示モード切替、レイアウト切替
│   │   ├── FileSidebar.tsx   # 変更ファイル一覧、ドキュメント一覧
│   │   ├── MainContent.tsx   # メインコンテンツ領域（3モード）
│   │   ├── DiffViewer.tsx    # 差分表示（Split/Stacked）
│   │   ├── MarkdownViewer.tsx # Markdown整形表示
│   │   └── ReviewPanel.tsx   # レビューコメント一覧、Copy All
│   ├── hooks/
│   │   ├── useDiff.ts        # 差分データ取得
│   │   └── useFileWatcher.ts # ファイル変更監視
│   ├── store/
│   │   └── index.ts          # Zustand グローバル状態管理
│   ├── types/
│   │   └── index.ts          # TypeScript 型定義
│   └── utils/
│       ├── tauri-api.ts      # Tauri バックエンド呼び出し
│       ├── format-review.ts  # レビュー出力フォーマット
