...
[Documentation](https://y-oga-819.github.io/tasuki/) | [Live Demo](https://y-oga-819.github.io/tasuki/demo) | [日本語版 / Japanese](#日本語)
...
```
  Claude Code 🏃 ──→ 🎌 Tasuki 🎌 ──→ 🏃 Human
     Work done       Show design docs    Review diffs with
                  ...
                  ←── cmux send ──←     Approve / Reject
                  Feedback auto-sent    with structured comments
                  back to Claude Code   back to Claude Code
```

## Features

- **Claude Code Integration** — Approve/Reject results are automatically sent back to Claude Code via [cmux](https://github.com/y-oga-819/cmux), closing the feedback loop
- **Commit Gate** — Claude Code Hook integration to block commits until review is approved
- **Diff Viewer** — GitHub-style Files Changed UI, Split/Unified toggle, Scroll/Wrap toggle, Staged/Unstaged filter
...
- **Design Doc Viewer** — Markdown & HTML rendering, Mermaid diagrams, auto-generated TOC
...
- **External Folder Browsing** — Add arbitrary folders to browse Markdown files
...
- **Bundled Claude Code Skills** — Includes planning and retrospective skills for AI-assisted development workflows
...
- **Claude Code 連携**: Approve/Reject の結果を [cmux](https://github.com/y-oga-819/cmux) 経由で Claude Code に自動送信し、フィードバックループを完結
- **コミットゲート**: Claude Code Hook と連携し、レビュー承認なしのコミットを防止
- **差分ビューア**: GitHub 風 Files Changed UI、Split/Unified 切り替え、Scroll/Wrap 切り替え、Staged/Unstaged フィルター
...
- **設計書ビューア**: Markdown・HTML 整形表示、Mermaid ダイアグラム描画、TOC 自動生成
