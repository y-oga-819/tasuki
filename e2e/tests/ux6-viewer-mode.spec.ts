...
test.describe("UX6: Viewer Mode & Mermaid Zoom", () => {
  ...
  test("clicking Viewer tab switches to viewer layout", async ({ page }) => {
    const viewerTab = page.locator("button.tab-btn").filter({ hasText: "Viewer" });
    ...
    const viewerLayout = page.locator(".main-content.viewer-layout");
  ...
  test("viewer mode hides diff-specific controls in toolbar", async ({ page }) => {
    ...
    const stats = page.locator(".toolbar-stats");
  ...
  test("viewer mode hides Changed Files in sidebar", async ({ page }) => {
    ...
    const changedSection = page.locator(".sidebar-section-title").filter({ hasText: "Changed Files" });
  ...
  test("viewer mode shows Add Folder button in sidebar", async ({ page }) => {
    ...
    const addFolderBtn = page.locator("button.add-folder-btn");
  ...
  test("viewer mode shows Documents section in sidebar", async ({ page }) => {
    ...
    const docsSection = page.locator(".sidebar-section-title").filter({ hasText: "Documents" });
  ...
  test("Mermaid zoom button opens modal and can be closed", async ({ page }) => {
    ...
    const mermaidBlock = page.locator("div.mermaid-block").first();
    ...
    const zoomBtn = mermaidBlock.locator("button.mermaid-zoom-btn");
    await zoomBtn.click();
    ...
    const level = page.locator(".mermaid-zoom-level");
    await expect(level).toHaveText("100%");

    // Close button should exist
    const closeBtn = page.locator("button.mermaid-zoom-close");
  ...
  test("Mermaid zoom controls work correctly", async ({ page }) => {
    ...
    // Click zoom in (+)
    const zoomInBtn = page.locator(".mermaid-zoom-ctrl-btn").filter({ hasText: "+" });
    ...
    // Click zoom out (-)
    const zoomOutBtn = page.locator(".mermaid-zoom-ctrl-btn").filter({ hasText: "-" });
